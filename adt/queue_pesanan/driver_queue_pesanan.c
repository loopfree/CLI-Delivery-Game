...
int main()
{
    ...
    // sorted_pesanan* sq = createsorted_pesanan();

    // pushsorted_pesanan(sq, variable1);
    // pushsorted_pesanan(sq, variable2);
    // pushsorted_pesanan(sq, variable3);
    ...
    sorted_pesanan* sq = makeSQ(arr, len);


    printf("\nSetelah di sort:\n");
    while(!isEmptysorted_pesanan(sq)) {
        jumlahPesanan out = popsorted_pesanan(sq);
